# 情绪像素 · MoodTracker 2.1.6

一个温柔、私密、无需注册的心情空间。保留 Expo / React Native 原生项目，支持 iOS、Android 和响应式网页。

**2.1.6 发布准备（2026-09-09）**：采用用户本轮确认的第三款 C「弯眼微笑」九宫格图标。网页版已部署并核验，现为同一 App 的 TestFlight「个人测试组」重新生成原生安装包。下方 2.1.5（9）为上一安装包的历史记录。

**2.1.5（9）发布记录（2026-09-08）**：产品更名「情绪像素」，采用已确认的彩色像素图标，并修复启动页的底色方块与加载衔接。220 项测试、独立原生编译、正式构建及 IPA 验签均通过；EAS 上传已于 03:42 UTC 完成。2026-09-09 已重新核实该上传任务为 `FINISHED`；测试组状态另行核对。
