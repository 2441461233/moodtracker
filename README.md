...
**2.1.10 正在发布：语音记录**：第三步默认录音，支持暂停、回听和云端 AI 转写，整理后的正文默认展示，原声保存在本机。含键盘弹层布局调整；正式包、发布提交的原生门槛和 TestFlight 分发待核实。自动转写还需独立配置 HTTPS 后端；当前发布机器没有后端凭据或已部署地址。后端配置、音频备份边界与验证见 [语音记录说明](docs/voice-recording.md)。
...
**2.1.5（9）发布记录（2026-09-08）**：产品更名「情绪像素」，采用已确认的彩色像素图标，并修复启动页的底色方块与加载衔接。220 项测试、独立原生编译、正式构建及 IPA 验签均通过；EAS 上传已于 03:42 UTC 完成。2026-09-09 已重新核实该上传任务为 `FINISHED`，原「个人测试组」显示该版本「正在测试」。
...
新增小号「快速记录」与大号「心情日历」：点小卡片直接打开记录弹框，点大卡片的日期查看当天本地心情。已包含在 2.1.4（8）的完整原生包中。更新后先打开 App 一次，再长按 iPhone 主屏幕添加小组件，搜索「情绪像素」（新版名称）。使用、数据边界和验收见 [iOS 桌面小组件](docs/ios-widgets.md)。
...
记录使用原有 AsyncStorage 键 `mood_entries`，旧版 emotionId / categoryId / note / timestamp 保持兼容。旧分类自动显示为对应活动，不会在启动时重写旧记录。补充文字最多 1,000 字，语音转写正文单独保存，最多 12,000 字；导入支持旧有的较长笔记，编辑时不会悄悄截断。
...
- 本地日记本体不在不同浏览器 / 设备间自动同步；文字与心情可导出 JSON 后导入，原声需要单独导出。Apple 健康中的心情由系统管理跨设备同步，不等同于迁移本地文字笔记。
- JSON 备份包含本地心情及转写文字，不包含音频文件、称呼、主题、健康读取结果或写入账本。CSV 用于查看，不是恢复格式。
- 云端语音转写需主动配置并开启；每次结束录音会经私人后端上传所选音频至硅基流动识别，再用 DeepSeek 整理语句。补充文字、活动、健康数据不参与转写。供应商密钥只在后端保存。
...
语音转写后端需单独部署到支持 Node.js 的 HTTPS 主机，不能运行于 GitHub Pages；本地启动用 `npm run voice:server`。`npm run verify` 也包含不访问真实供应商的后端回归。

Pull Request 只检查构建，不发布。Pages 使用 GitHub Actions 工作流模式；基础日记无需服务器或数据库，可选语音转写需要单独后端。
...
应用采用已确认的「情绪像素」图标，显示名称为「情绪像素」。图标与分享图由 ImageGen 生成；`assets/app-icon.png` 为主图标，`brand-icon.png` 为界面内优化尺寸，`pwa-icon.png` 用于安装，`og.png` 用于分享预览。界面图标来自 MaterialCommunityIcons；趋势线是实际数据图表，不是装饰插画。

原生启动画面通过 `expo-splash-screen` 使用与首页相同的浅、深色纯色背景，暂不显示带不透明底色的图标。系统启动画面会保留到本地记录及外观设置读取完成、首页或错误恢复页完成布局，再直接进入界面。之后如需显示启动 Logo，可另行加入通过 alpha 检查的独立透明标志。启动配置变更需重新生成并安装原生包；在正式构建中检查浅色、深色冷启动及存储读取失败时的恢复页，Expo Go 不能完整还原最终启动效果。
