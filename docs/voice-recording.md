...
语音实现基于远端 main `9417969`（2026-09-09 21:28）开发，提交为 `0f821a6`。2026-09-10 已随 **2.1.11（17）** 发布到网页及原 TestFlight「个人测试组」，包含键盘遮挡、播放器释放顺序和录音到回听切换修复；精确证据见 [发布记录](ios-release.md)。**云端转写后端尚未接通确认**，需找回原开发机配置及已部署的 HTTPS 地址。第三步默认进入语音记录：点击开始，可暂停、继续，结束后先保存本机原声，已配置并开启服务时再自动转写。参考 [flomo 语音笔记](https://help.flomoapp.com/ai/aivoice.html) 的轻操作与正文呈现方式，AI 整理结果默认展开，识别原文折叠，原声提供回听与导出。文字输入作为独立补充保留。

## 服务配置

云端部署的容器、环境变量与验收步骤见 [服务部署说明](../server/DEPLOY.md)。录音停止闪退的排查、已发布修复与原生测试证据见 [问题记录](voice-stop-crash-2026-09-10.md)。
...
3. 打包时更新版本号与构建号，并完成原生构建。本次发布版本已更新为 2.1.11；构建及分发的精确状态见 [发布记录](ios-release.md)。
...
2026-09-10 的上一版本 2.1.10 源码 `aa004ea` 通过 TypeScript、216 项应用测试、5 项服务测试、19 项网页检查和生产 Web 构建。云端 iOS 26.2 模拟器 Release App 的浅色 / 深色两条录入流程通过，涵盖默认语音入口、切换文字、正文保留、键盘、输入框完整可见及保存；原始截图已人工复核。正式 IPA 主 App 和小组件均通过严格验签，确认中文麦克风说明及 ExpoAudio / ExpoSecureStore 模块存在。**这些结果不代替真实麦克风录音、真机回听、云端转写或 iPhone 帧率验收；上述项目仍待完成。**

2.1.11（17）客户端源码 `9d2d5e4` 修复播放器生命周期与音频模式切换，后续 `e3d0397` 仅调整测试、服务和文档。250 项应用 / 服务 / 网页回归通过；最终原生 XCTest 三项通过、零失败 / 跳过，包含实际录音、暂停续录、结束、回听、切换文字、保存和再次打开，以及浅深色键盘完整可见。无签名模拟器的 SecureStore 读取失败被保留为可恢复状态，仍可保存原声；不能据此声称正式签名包钥匙串或云端转写已验收。Apple 已确认新包在原内部组正在测试，真机反馈是否完全解决仍需用户更新后复测。
