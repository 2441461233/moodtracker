...
本次新增原生 WidgetKit 扩展，包含两张独立卡片。代码检查不代表已安装到手机；需要重新构建并安装包含扩展的 iOS App，Expo Go、网页和 JS 热更新不能添加这些小组件。
...
- 模拟器压缩包 19,018,075 字节，SHA-256：`7b131d8491018436bd63a7b453b3a8d2560f4ade19c42422e66fbfc006c9a237`。本机严格 codesign 完整性检查通过。该模拟器产物是 ad hoc 签名、签名 entitlement 为空，**不能验证真机 App Group 签名或实际摘要读写**；发布前仍须配置并核验主 App / 扩展的 provisioning profiles。
- 尚未做真机安装、WidgetKit 视觉和交互验收，也未发布这次功能到 TestFlight。

## 使用

1. 安装新版后打开「心情日记」一次，让本地日历摘要写入共享空间。
2. 长按 iPhone 主屏幕空白处，选择添加小组件，搜索「心情日记」。
...
- 插件写入 EAS appExtensions 元数据。当前生产 profile 使用本地签名：发布前需要给**主 App 更新含 App Group 的 provisioning profile，并为扩展准备独立的 profile**；本地凭据需按 EAS 的多 target 格式配置。既有单 target 凭据不能直接签这个版本。没有修改或生成任何签名材料。
...
真机待验收：添加两种尺寸；浅色 / 深色 / 着色模式与大字体；小卡片的冷启动、后台恢复、已有草稿、关闭其他弹框；日历点击、重复点击同一天、月末与跨年、保存 / 修改日期 / 删除最后一条 / 导入后刷新；首次解锁前后的摘要恢复；主 App 和扩展的实际签名及 TestFlight 安装。未完成前不标记已发布或真机通过。
